...
Next, tell Capistrano about your external dependencies. This is done via a YAML file: config/externals.yml. It describes a hash of path names that describe the external repository. It is ERB-evaluated before being loaded so you can include ERB tags to loop over long lists of dependencies. For example:
...
This recipe introduces a special kind of SCM: _local_. It allows for checking out external dependencies located on your filesystem, which may not be version controlled, or are hosted on a server that is not reachable by your deployment servers. When setting up externals locally, a symlink is created to point to the dependency's directory. They are deployed by uploading a compressed archive of that directory on each server instead of having them download the directory themselves. Note that _local_ dependencies cannot be specified a revision since it would not make sense. Example usage:
